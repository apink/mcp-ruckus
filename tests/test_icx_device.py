...
class TestLLdpNeighbors:
    ...
    def test_returns_list(self):
        result = _device_lldp_neighbors("10.60.172.1")
        assert isinstance(result, list)
...
class TestDeviceResources:
    ...
    def test_valid_device(self):
        result = _device_resources("10.60.172.1")
...
class TestSfpInfo:
    ...
    def test_valid_device(self):
        result = _device_sfp_info("10.60.172.1")
        assert isinstance(result, list)
        assert len(result) == 2
        assert result[0]["port"] == "1/2/1"
...
class TestCableDiag:
    ...
    def test_valid_device(self):
        result = _device_cable_diag("10.60.172.1", "1/1/1")
        assert result["host"] == "10.60.172.1"
...
class TestSyslog:
    ...
    def test_valid_device(self):
        result = _device_syslog("10.60.172.1")
        assert result["host"] == "10.60.172.1"
    ...
    def test_severity_filter(self):
        result = _device_syslog("10.60.172.1", severity="E")
...
class TestOpticInfo:
    ...
    def test_valid_device(self):
        result = _device_optic_info("10.60.172.1", "1/2/1")
...
class TestDeviceTime:
    ...
    def test_valid_device(self):
        result = _device_time("10.60.172.1")
        assert result["host"] == "10.60.172.1"
...
class TestSpanningTree:
    ...
    def test_valid_device(self):
        result = _device_spanning_tree("10.60.172.1")
...
class TestAccessLists:
    ...
    def test_all_acls(self):
        result = _device_access_lists("10.60.172.1")
        assert result["host"] == "10.60.172.1"
    ...
    def test_brief(self):
        result = _device_access_lists("10.60.172.1", brief=True)
        assert len(result["acls"]) == 2
...
class TestUsers:
    ...
    def test_valid_device(self):
        result = _device_users("10.60.172.1")
        assert isinstance(result, list)
        assert len(result) == 1
...
class TestSshStatus:
    ...
    def test_valid_device(self):
        result = _device_ssh_status("10.60.172.1")
...
class TestPortState:
    ...
    def test_enable_port(self):
        result = _device_port_state("10.60.172.1", "1/1/24", enable=True)
        assert result["port"] == "1/1/24"
        assert result["state"] == "enabled"

    def test_disable_port(self):
        result = _device_port_state("10.60.172.1", "1/1/24", enable=False)
        assert result["port"] == "1/1/24"
...
class TestVlanCreate:
    ...
    def test_single_vlan(self):
        r = _device_vlan_create("10.60.172.1", "200", name="TEST_VLAN")
        assert r["vlan_spec"] == "200"
        assert r["vlan_ids"] == [200]
        assert r["created"] is True

    def test_vlan_range(self):
        r = _device_vlan_create("10.60.172.1", "210 to 213")
        assert r["vlan_ids"] == [210, 211, 212, 213]

    def test_vlan_multi(self):
        r = _device_vlan_create("10.60.172.1", "200 210 220")
        assert r["vlan_ids"] == [200, 210, 220]

    def test_vlan_mixed(self):
        r = _device_vlan_create("10.60.172.1", "16 17 20 to 24")
...
class TestVlanDelete:
    ...
    def test_delete_single(self):
        r = _device_vlan_delete("10.60.172.1", "200")
        assert r["deleted"] is True
        assert r["vlan_ids"] == [200]

    def test_delete_range(self):
        r = _device_vlan_delete("10.60.172.1", "200 to 205")
...
class TestVlanPort:
    ...
    def test_add_tagged(self):
        r = _device_vlan_port("10.60.172.1", "1/1/1", "200", "add", tagged=True)
        assert r["success"] is True
        assert r["action"] == "add"

    def test_remove(self):
        r = _device_vlan_port("10.60.172.1", "1/1/1", "200", "remove")
        assert r["success"] is True
...
class TestDryRun:
    def test_port_state_dry_run(self):
        r = _device_port_state("10.60.172.1", "1/1/1", enable=False, dry_run=True)
        assert r["dry_run"] is True
        assert "commands" in r
        assert len(r["commands"]) >= 2

    def test_vlan_create_dry_run(self):
        r = _device_vlan_create("10.60.172.1", "100", name="TEST", tagged_ports="ethernet 1/1/1",
    ...
    def test_vlan_delete_dry_run(self):
        r = _device_vlan_delete("10.60.172.1", "100", dry_run=True)
        assert r["dry_run"] is True
        assert "commands" in r
        assert len(r["commands"]) >= 2

    def test_vlan_port_dry_run(self):
        r = _device_vlan_port("10.60.172.1", "1/1/1", "200", action="add", dry_run=True)
        assert r["dry_run"] is True
        assert "commands" in r
        assert len(r["commands"]) >= 2
...
class TestPoePort:
    ...
    def test_enable(self):
        r = _device_poe_port("10.60.172.1", "1/1/1", enable=True)
        assert r["success"] is True
        assert r["action"] == "enable"

    def test_disable(self):
        r = _device_poe_port("10.60.172.1", "1/1/1", enable=False)
        assert r["action"] == "disable"

    def test_dry_run(self):
        r = _device_poe_port("10.60.172.1", "1/1/1", enable=True, dry_run=True)
        assert r["dry_run"] is True
        assert "commands" in r

    def test_enable_with_priority(self):
        r = _device_poe_port("10.60.172.1", "1/1/1", enable=True, priority=2)
        assert r["success"] is True
        assert r["action"] == "enable"
