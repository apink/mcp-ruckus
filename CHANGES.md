...
|---|---|---|
| 2026-09-09 | New tool: controller system statistics | `controller_statistics` (read-only) — new vSZ tool exposing controller system statistics via `GET /controller/{id}/statistics` (CPU %, disk total/free, memory %, per-port `control`/`cluster`/`management`/`portN` traffic) with `interval` (QUARTER/HOUR/DAY) + `size`; `adapters/vsz.py` gains `controller_statistics()` (auto-resolves controller id from `/controller`) + `_validate_uuid()` (closes the S2 URL-injection gap for this endpoint); tool count 90 → 91 (vSZ 32), tests 362 → 369 |
| 2026-09-09 | Docs: deployment hierarchy + data cleanup | `README.md`/`docs/QUICKSTART.md` — quick start re-framed to a clear three-tier preference: Docker (production, recommended) → systemd (native Linux production) → `run.py` (simple launcher for macOS/Windows/Linux-without-systemd); `deploy/native.md` + `deploy/systemd.md` add honest positioning notes (`run.py` has no crash auto-restart); removed personal/location data from docs (`README.md` credits, `docker-compose.yml` default `TZ`, and example locations in `inventory/devices.example.yaml`) |
| 2026-09-09 | Native cross-platform launcher + config-driven restart | New `run.py` (stdlib-only) launcher — `start\|stop\|restart\|status` on Linux/macOS/Windows via PID files + `taskkill`/SIGTERM; `admin.py` **Restart MCP** now runs the configurable `MCP_RESTART_CMD` (falls back to legacy `MCP_SYSTEMD_UNIT`); new `deploy/native.md` (launchd/Task Scheduler/cron); `README.md`/`docs/QUICKSTART.md`/`.env.example`/`deploy/systemd.md` synced |
| 2026-09-08 | API key scope: explicit + default lean | `admin.py` — key form now has an explicit `Scope` choice (**Selected tools** / **All tools**) instead of the implicit "empty = all tools"; new keys default to **Selected tools** with **Most used tools** pre-filled; an inline warning shows when a key would grant all tools; `static/app.js` + `static/style.css` updated; `README.md`/`docs/QUICKSTART.md`/`SECURITY.md` synced |
| 2026-09-08 | Lean agent toolset + scoped `tools/list` | `security.py` — `AuditMiddleware.on_list_tools` filters the tool list to what the current key may call (scoped keys no longer receive all 90 schemas); new `lean.py` `LEAN_TOOLS` (~17 read-only tools) plus a **Most used tools** preset in the API-key form; `SECURITY.md`/`README.md`/`docs/QUICKSTART.md` documented |
